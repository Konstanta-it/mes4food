...
|партии.Номенклатура|Товары.Номенклатура|
|партии.Количество|Товары.Количество|
|партии.партия|Товары.партия|
...
!!! info ""
    ...
    Загрузка табличной части *Товары* из табличной части *партии* происходит следующим образом: среди уже загруженных строк ищется строка по Номенклатуре и пустой партии. Если такая строка найдена, то она перезаполняется, если такая строка не найдена - добавляется новая
